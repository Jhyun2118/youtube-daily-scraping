...
# ── 롱폼 타깃 dose 추적 (Stage 3b) ────────────────────────────────
# Stage 3(활성 숏)은 거의 숏→숏만 임베드 → 롱폼(>180s) 타깃이 embeds_daily 일별 추적에서 빠짐
# (숏→롱 funnel 식별 불가). 롱폼을 임베드하는 숏들을 매일 일부씩(least-recently-seen 우선)
# 재방문해 롱폼 타깃의 일별 dose(연결 숏 수) 변동을 포착. CAP=시간 가드(0이면 비활성).
LONGFORM_DOSE_REPARSE_CAP = 3000
LONGFORM_DURATION_THRESHOLD = 180     # 이 길이(초) 초과 = 롱폼 (Shorts 하드캡)
